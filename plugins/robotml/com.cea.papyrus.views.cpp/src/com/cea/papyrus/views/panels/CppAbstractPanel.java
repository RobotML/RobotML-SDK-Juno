...
import com.cea.papyrus.views.listeners.UMLElementPanelListener;
...
public abstract class CppAbstractPanel extends Composite {
	...
	protected IDocument createDocumentC() {
		...
		// tests for eventual future integration of a CDT editor
		/*
		 * // CEditor tst = new CEditor();
		 * IWorkbenchPage page = PlatformUI.getWorkbench().getActiveWorkbenchWindow().getActivePage();
		 * IEditorDescriptor desc = PlatformUI.getWorkbench().
		 * getEditorRegistry().getDefaultEditor("tst.cpp");
		 * IWorkspaceRoot wsRoot = ResourcesPlugin.getWorkspace().getRoot();
		 * try {
		 * IProject prj = wsRoot.getProject("TestCppProjectSimple");
		 * IFile ifile = prj.getFile("test.cpp");
		 * System.out.println (ifile.getFullPath ());
		 * if (ifile.exists ()) {
		 * IFileEditorInput ei = new FileEditorInput(ifile);
		 * try {
		 * IEditorPart ed = page.openEditor(ei, desc.getId());
		 * // alternative:
		 ...
		 * }
		 * 
		 * // CCorePlugin.getPDOMManager().
		 * ICElement element = CoreModel.getDefault().create(ifile);
		 * IASTTranslationUnit unit = null;
		 * // returned element is ITranslationUnit, not IASTTranslationUnit
		 * if (element instanceof IASTTranslationUnit)
		 * {
		 * unit = (IASTTranslationUnit) element;
		 * }
		 * if (unit == null) {
		 * unit = CDOM.getInstance().getTranslationUnit(ifile);
		 * }
		 * // unit = LanguageManager.getInstance().getLanguage("cpp").getASTTranslationUnit(reader, scanInfo, fileCreator, index, log)
		 * String output = "";
		 * for (IASTDeclaration decl : unit.getDeclarations())
		 * {
		 * if (decl instanceof IASTFunctionDeclarator) {
		 * IASTFunctionDeclarator fd = (IASTFunctionDeclarator) decl;
		 * for (IASTNode child : fd.getChildren()) {
		 * if (decl instanceof IASTFunctionDeclarator) {
		 ...
		 * }
		 * }
		 * output += "Function declaration: " + ((IASTFunctionDeclarator)decl).getName();
		 * }
		 ...
		 * System.out.println (child);
		 * }
		 * output += "Function definition: " + fd.getRawSignature();
		 * }
		 * else if (decl instanceof IASTExpression) {
		 * output += "Expression: " + ((IASTExpression)decl).getName();
		 * }
		 * }
		 * }
		 */
